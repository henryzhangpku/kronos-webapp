            ...
            for attempt in range(3):  # Try 3 times for each symbol
                try:
                    ...
                    if not hist_data.empty and len(hist_data) > 50:
                        ...
                        logger.info(f"✅ Successfully fetched {len(hist_data)} real data points using symbol: {yf_symbol}")
                        break
                    else:
                        logger.warning(f"Insufficient data returned for symbol: {yf_symbol} (got {len(hist_data)} points)")
                        hist_data = pd.DataFrame()  # Reset for next attempt
